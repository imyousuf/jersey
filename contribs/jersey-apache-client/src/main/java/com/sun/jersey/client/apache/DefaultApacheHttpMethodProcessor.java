...
/**
 * An Apache Http Method processor.
 * @author imyousuf@smartitengineering.com
 */
public class DefaultApacheHttpMethodProcessor extends TerminatingClientHandler implements ApacheHttpMethodProcessor {
  ...
  private DefaultApacheHttpMethodProcessor(HttpClient httpClient, ClientConfig clientConfig,
                                           ThreadLocal<ClientRequest> request) {
    this.httpClient = httpClient;
    this.clientConfig = clientConfig;
    this.request = request;
    this.httpClient.getParams().setAuthenticationPreemptive(this.clientConfig.getPropertyAsFeature(
        ApacheHttpClientConfig.PROPERTY_PREEMPTIVE_AUTHENTICATION));
    final Integer connectTimeout = (Integer) this.clientConfig.getProperty(
  ...
  public static ApacheHttpMethodProcessor getProcessorInstance(HttpClient httpClient, ClientConfig clientConfig) {
    return getProcessorInstance(httpClient, clientConfig, null);
  }

  public static ApacheHttpMethodProcessor getProcessorInstance(HttpClient httpClient, ClientConfig clientConfig,
                                                               ThreadLocal<ClientRequest> request) {
    return new DefaultApacheHttpMethodProcessor(httpClient, clientConfig, request);
  }

  @Override
  public void executeMethod(final HttpMethod method) {
    if (request == null || request.get() == null) {
      throw new IllegalStateException("No ThreadLocal request provider set!");
    }
    executeMethod(method, request.get());
  }
